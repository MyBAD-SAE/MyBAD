...
defineProps({
    ...
    iconColorClass: { type: String, default: 'text-muted-foreground' },
});
</script>
...
<template>
    <Link
        :href="href"
        class="group relative flex flex-col rounded-2xl border bg-white p-5 transition-all"
    >
        <div class="flex items-start justify-between">
            ...
            <span
                v-if="badge !== null"
                class="inline-flex h-6 min-w-6 items-center justify-center rounded-full bg-gray-100 px-2 text-xs font-semibold text-foreground"
            >
                {{ badge }}
            </span>
        </div>
